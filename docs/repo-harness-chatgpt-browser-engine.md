...
Multi-turn works two ways: repeat `--follow-up` within one run, or reopen a saved conversation later with `browser-followup --session <id>`. Prompt-bearing follow-ups start a new Oracle run at `--chatgpt-url <exact-conversation-url>` instead of Oracle's `--followup`, because Oracle 0.16.1 clears its tab reference on that path. This avoids dependence on whichever DevTools tab is active; repo-harness validates the returned conversation and fails closed on a mismatch. For legacy local sessions without a projected URL, it resolves the URL from saved Oracle provider metadata. On a proven pre-submit attach failure, the wrapper reopens the source provider session read-only and retries once. A follow-up records the parent `providerSessionId`, only resumes from a session that reached a resumable terminal state, and uses the binding recorded on the parent repo-harness session.
